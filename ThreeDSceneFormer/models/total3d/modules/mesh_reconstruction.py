            ...
            if self.train_e_e:
                ...
                for batch_id in range(n_batch):
                    ...
                    if len(rm_edges)>0:
                        ...
                        if mode == 'test':
                            current_faces[batch_id][sphere_edge2face[rm_edges].sum(0).type(torch.bool), :] = 1 # type: ignore
        ...
        if cuda:
            ...
            for s in structured_implicit.unbind():
                if self._temp_folder is None:
                    ...
                    # Use package-relative path for ldif2mesh binary
                    ldif2mesh_src = os.path.join(os.path.dirname(__file__), '..', '..', '..', 
                                                   'external', 'ldif', 'ldif2mesh', 'ldif2mesh')
                    shutil.copy(ldif2mesh_src, self._temp_folder)
