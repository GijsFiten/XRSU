...
class TransformerEncDec(nn.Module):
    ...
    def forward(self, output):
        ...
        if self.cfg.config['model']["output_adjust"]["use_pos_enc"]:
            src_enc = self.pos_enc_src.forward(x_pred)
            tgt_enc = self.pos_enc_tgt.forward(x_obj_lo)

            x_pred += src_enc
            x_obj_lo += tgt_enc

        x_pred = torch.reshape(x_pred, (x_pred.shape[1], x_pred.shape[0], x_pred.shape[2]))
        x_obj_lo = torch.reshape(x_obj_lo, (x_obj_lo.shape[1], x_obj_lo.shape[0], x_obj_lo.shape[2]))

        query = self.transformer(x_pred, x_obj_lo)

        query = torch.reshape(query, (query.shape[1], query.shape[0], query.shape[2]))
        ...
        obj_feats_lo = query[num_obj:num_obj+num_lo]
