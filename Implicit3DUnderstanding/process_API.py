...
# Support running as top-level module or as Implicit3DUnderstanding.process_API
try:
    from net_utils.utils import load_device, load_model, CheckpointIO
...
def load_2D_model(model_2d: str='Resnet50'):
    if model_2d not in ['Resnet50', 'Mobilenetv2', 'DETR']:
        raise NotImplementedError(f"Model {model_2d} is not implemented yet.")
    model = torchvision.models.detection.fasterrcnn_resnet50_fpn()
    in_features = model.roi_heads.box_predictor.cls_score.in_features
    model.roi_heads.box_predictor = FastRCNNPredictor(in_features, 41)
    # Resolve path relative to repo root
    this_dir = os.path.dirname(__file__)
    weights_path = os.path.normpath(os.path.join(this_dir, '..', '2D_Detectors', model_2d, 'fine_tuned_resnet.pth'))
...
def get_bounding_boxes(photo, threshold):
    ...
    with torch.no_grad():
        prediction = objectDetector(image_tensor) # type: ignore
...
def initiate(cfg_begin):
    ...
    objectDetector = load_2D_model(cfg.config['bb_model'])
...
def run(image, camera):
    ...
    lo_bdb3D_out = get_layout_bdb_sunrgbd(cfg.bins_tensor, est_data['lo_ori_reg_result'], # type: ignore
    ...
    cam_R_out = get_rotation_matix_result(cfg.bins_tensor, # type: ignore
    ...
    bdb3D_out_form_cpu, bdb3D_out = get_bdb_evaluation(cfg.bins_tensor, # type: ignore
    ...
    # save results
    nyu40class_ids = [int(evaluate_bdb['classid']) for evaluate_bdb in bdb3D_out_form_cpu] # type: ignore
    ...
    return_dict["bdb"] = bdb3D_out_form_cpu[interval[0]:interval[1]] # type: ignore
    return_dict['class_id'] = current_cls
    return_dict["cam_R"] = cam_R_out[0, :, :].cpu().numpy() # type: ignore
