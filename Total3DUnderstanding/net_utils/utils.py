...
class CheckpointIO(object):
    ...
    def finetune(self):
        ...
        for weight_path in weight_paths:
            if not os.path.exists("Total3DUnderstanding/" + weight_path):
                self.cfg.log_string('Warning: finetune failed: the weight path %s is invalid. Begin to train from scratch.' % ("Total3DUnderstanding/" + weight_path))
            else:
                self.load("Total3DUnderstanding/" + weight_path, 'net')
